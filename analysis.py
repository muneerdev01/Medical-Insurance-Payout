...
try:
    ...
    # Fallback to generate dummy clean data if running standalone without the file
    np.random.seed(42)
    n_samples = 1338
    ...
    # Base formula for realistic medical charges
    charges = (ages * 250) + (bmis * 350) + (children * 500)
    charges += np.where(smokers == 'yes', 25000 + (bmis * 500), 0)
    charges += np.random.normal(0, 1500, n_samples)
    charges = np.maximum(charges, 1000) # Minimum charge $1000
...
for idx, row in feature_importance.reset_index().iterrows():
...
│  [Age vs Charges Scatter]   [BMI Category Breakdown]    │
└─────────────────────────────────────────────────────────┘

[RISK SEGMENTATION TAB]
  • Heatmaps of Age vs BMI for Smoker / Non-Smoker cohorts.
  • Feature Importance Bar Chart showing key risk coefficients.
"""
print(dashboard_structure)
print("\n" + "=" * 80)
print("END OF BUSINESS INTELLIGENCE REPORT")
print("=" * 80)
